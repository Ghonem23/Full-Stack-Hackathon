...
function DoctorRobot({ className }) {
  return (
    ...
    >
      ...
      <rect x="87" y="59" width="126" height="108" rx="37" stroke="#ffffff" strokeOpacity="0.7" strokeWidth="2" />
      ...
      {/* Smile */}
      <path d="M133 134q17 11 34 0" stroke="#5fdcf2" strokeWidth="4" strokeLinecap="round" opacity="0.85" />

      {/* Neck */}
      <rect x="134" y="163" width="32" height="22" rx="9" fill="#c3d6ea" />

      {/* Doctor coat */}
      <path d="M118 187 150 216 182 187c34 15 54 55 54 107v46H64v-46c0-52 20-92 54-107Z" fill="url(#dr-coat)" />
      ...
      {/* Stethoscope */}
      <path d="M126 192c-14 42-4 76 18 88" stroke="#0493ae" strokeWidth="6" strokeLinecap="round" />
      ...
      <path d="M190 246h5l3.5-8 4.5 16 3.5-8h11" stroke="#ffffff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function LoginForm({ onForgotPassword, onSuccess, onNavigate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (onSuccess) {
          onSuccess({ email: email.trim(), remember });
        }
      }, 1000);
    } catch (err) {
      setErrors({ form: "Unable to connect to the backend server." });
      setStatus("idle");
    }
  };
  ...
  if (status === "success") {
    return (
      <div className="nm-state">
        ...
        <h2 className="nm-state-title">Welcome back</h2>
  ...
  return (
    <form className="nm-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="nm-field">
        <label className="nm-label" htmlFor="nm-email">
          Email address
        </label>
        <div className={`nm-input-wrap ${errors.email ? "nm-input-wrap--error" : ""}`}>
          <Mail size={17} className="nm-input-icon" />
          <input
            id="nm-email"
            type="email"
            autoComplete="email"
      ...
      <div className="nm-field">
        ...
        <div className={`nm-input-wrap ${errors.password ? "nm-input-wrap--error" : ""}`}>
          <Lock size={17} className="nm-input-icon" />
          <input
            id="nm-password"
      ...
      <div className="nm-row-between">
        <label className="nm-checkbox">
          ...
          <span className="nm-checkbox-label">Remember me</span>
        </label>

        <button type="button" className="nm-link-btn" onClick={onForgotPassword}>
          Forgot password?
        </button>
      </div>

      {/* LOGIN BUTTON */}
      <button type="submit" className="nm-submit" disabled={status === "loading"}>
        {status === "loading" ? (
          <>
            <Loader2 size={17} className="nm-spin" />
            Signing in…
          </>
        ) : (
          <>
...
function ForgotPassword({ onBack }) {
  ...
  if (status === "success") {
    return (
      <div className="nm-state">
        ...
        <h2 className="nm-state-title">Check your inbox</h2>
        <p className="nm-state-text">
          Password reset instructions have been sent to your email.
        </p>
        <button type="button" className="nm-secondary-btn" onClick={onBack}>
          <ArrowLeft size={15} />
          Back to sign in
        </button>
      </div>
  ...
  return (
    <form className="nm-form" onSubmit={handleSubmit} noValidate>
      <button type="button" className="nm-back-btn" onClick={onBack}>
        <ArrowLeft size={15} />
        Back to sign in
      </button>
      ...
      <div className="nm-field">
        <label className="nm-label" htmlFor="nm-reset-email">
          Email address
        </label>
        <div className={`nm-input-wrap ${error ? "nm-input-wrap--error" : ""}`}>
          <Mail size={16} className="nm-input-icon" />
          <input
            id="nm-reset-email"
            type="email"
            autoComplete="email"
...
export default function AuthCard({ onSuccess, onNavigate }) {
  const [view, setView] = useState("login");

  const cardTitle = view === "login" ? "Welcome back" : "Reset your password";
  const cardSubtitle =
    view === "login"
      ...
      <div className="nm-visual">
        ...
        <div className="nm-visual-copy">
          ...
          <p className="nm-visual-text">
            Ask about the relationship between depression and the immune system,
            and get an answer drawn from published research — in plain language.
          </p>
        </div>
      ...
      <div className="nm-panel">
        <div className="nm-panel-inner">
          <div className="nm-panel-head">
            <h1 className="nm-title">{cardTitle}</h1>
            {cardSubtitle && (
              <p className="nm-panel-subtitle">{cardSubtitle}</p>
            )}
          </div>
